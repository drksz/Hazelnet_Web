...
namespace HazelNet.Tests.Optimizer
{
    ...
    public class FsrsOptimizerEfIntegrationTests : IDisposable
    {
        ...
        public async Task EndToEnd_OptimizeWeightsAsync_ReadsFromRealDb_AndTrains()
        {
            ...
            using (var seedCtx = new SqliteTestDbContext(_options))
            {
        ...
        // A thin ApplicationDbContext subclass that makes the model buildable on SQLite.
        // PR #82 added User.FSRSParameters (owns double[] W — Npgsql-specific, no SQLite converter)
        // and Card.ReviewHistoryId (scalar that shadows the explicit 1:1 FK config). Both are
        // irrelevant to the optimizer tests, so we ignore them here rather than modifying infra.
